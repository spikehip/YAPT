...
private static final String INSERT = "insert into " + TABLE_NAME + "(ts_date, is_period, took_pill, strength, notes) values (?, ?, ?, ?, ?)";
...
public long insert(Date tsDate, boolean isPeriod) throws SQLiteConstraintException {
...
public List<PeriodEntity> selectAll(boolean showAll) {
    ...
    Cursor cursor = this.db.query(TABLE_NAME, new String[] { "ts_date","is_period","took_pill", "strength", "notes" },
               showAll?"":"is_period=1", null, null, null, "ts_date");
    if (cursor.moveToFirst()) {
        do {
            PeriodEntity pe = new PeriodEntity();
            try {
				pe.setTs_date(format.parse(cursor.getString(0)));
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				pe.setTs_date(null);
			}
            pe.setIs_period(cursor.getInt(1)==1?true:false);
            list.add(pe);
...
public boolean isPeriodDay(Date tsDate) {
            Cursor cursor = this.db.query(TABLE_NAME, new String[] {"ts_date" }, "ts_date=?", new String[] {format.format(tsDate)}, null, null, null);
